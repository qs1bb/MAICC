...
class QLearningTable:
    ...
    def learn(self, s, a, r, s_):
        ...
        eligibility = list(
            self.e_table[self.e_table >= self.e_min].stack().index)
        # print('Delta: ', delta)
...
class QAgent(Agent):
    ...
    def expand(self, obs, depth):
        ...
        player_no = next(
            (i for i, item in enumerate(obs.players) if item.is_self))
    ...
    def step(self, obs):
        ...
        action = rl_action if rl_action in obs.actions else random.choice(
            obs.actions)
