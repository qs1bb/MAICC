...
class PACCritic(nn.Module):
    ...
    def forward(self, batch, t=None, compute_all=False):
        if compute_all:
            inputs, bs, max_t, other_actions = self._build_inputs_all(
                batch, t=t)
        else:
            inputs, bs, max_t, other_actions = self._build_inputs_cur(
                batch, t=t)
    ...
    def _build_inputs_all(self, batch, t=None):
        ...
        if self.args.obs_last_action:
            if t == 0:
                ...
                inputs.append(
                    batch["actions_onehot"][:, slice(
                        t - 1, t)].view(bs, max_t, 1, -1)
                )
            else:
                last_actions = th.cat(
                    [
