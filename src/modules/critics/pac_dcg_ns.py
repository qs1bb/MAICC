...
class DCGCriticNS:
    ...
    def __init__(self, scheme, args):
        ...
        self.payoff_fun = [
            self._mlp(2 * self.args.hidden_dim,
                      args.cg_payoffs_hidden_dim, payoff_out)
    ...
    def annotations(self, ep_batch, t, compute_grads=False, actions=None):
        ...
        with th.no_grad() if not compute_grads else contextlib.suppress():
            ...
            for i, f in enumerate(self.utility_fun):
                f_i.append(f(self.hidden_states[i]).reshape(
            ...
            if len(self.payoff_fun) > 0:
                ...
                f_ij = f_i.new_zeros(
                    *f_i.shape[:-2], 0, self.n_actions, self.n_actions)
        return f_i, f_ij
    ...
    def single_payoff(self, payoff_fun, edge, hidden_states):
        ...
        if self.payoff_decomposition:
            ...
            output = th.bmm(
                output[:, 0, :].unsqueeze(
                    dim=-1), output[:, 1, :].unsqueeze(dim=-2)
            )
    ...
    def q_values(self, f_i, f_ij, actions):
        ...
        if len(self.edges_from) > 0:
            f_ij = f_ij.view(
                n_batches, len(self.edges_from), self.n_actions *
                self.n_actions
            )
    ...
    def load_state_dict(self, other_mac):
        ...
        for i in range(len(self.utility_fun)):
            self.utility_fun[i].load_state_dict(
                other_mac.utility_fun[i].state_dict())
        for i in range(len(self.payoff_fun)):
            self.payoff_fun[i].load_state_dict(
                other_mac.payoff_fun[i].state_dict())
    ...
    def _mlp(input, hidden_dims, output):
        ...
        hidden_dims = [hidden_dims] if isinstance(
            hidden_dims, int) else hidden_dims
        dim = input
        layers = []
    ...
    def forward(
        ...
        if policy_mode:  # ... either as policy tensor for the runner ...
            policy = f_i.new_zeros(ep_batch.batch_size,
