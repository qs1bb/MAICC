...
def _compute_combinations(config_file, shuffle, seeds):
    ...
    click.echo(
        click.style(" X ", fg="red", bold=True).join(
            [str(s) for s in combinations])
    )
...
def locally(combos, cpus):
    configs = ["python main.py " + " ".join([c for c in combo if c.startswith("--")]) + " with " + " ".join([
...
def single(combos, index):
    ...
    config = combos[index]
    cmd = "python main.py " + " ".join([c for c in config if c.startswith(
        "--")]) + " with " + " ".join([c for c in config if not c.startswith("--")])
    print(cmd)
    work(cmd)
