...
from components.episode_buffer import ReplayBuffer, InContextReplayBuffer
...
def evaluate_sequential(args, runner, buffer, mac):
    if args.name == "IC-MARL":
        buffer.clean_online()
...
def run_sequential(args, logger):
    ...
    if args.name == "IC-MARL":
        mac = mac_REGISTRY[args.mac](env_info, buffer, args)
    else:
    ...
    if args.name == "IC-MARL" and args.train_mode == 1:
    ...
    if args.checkpoint_path != "":
        ...
        if args.name == "IC-MARL" and args.train_mode == 1:
            args.t_max *= 2
            buffer.set_agent(mac.agent)
            buffer.init_embedding()

        runner.t_env = timestep_to_load

        if args.evaluate or args.save_replay:
            runner.log_train_stats_t = runner.t_env
            evaluate_sequential(args, runner, buffer, mac)
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):
            ...
            if args.name == "IC-MARL":
                runner.t_env += int(args.batch_size)
                learner.train(episode_sample, runner.t_env,
                              episode, buffer, args.train_mode)
            else:
        ...
        if args.save_model and (
            ...
            if args.name == "IC-MARL":
                save_path = os.path.join(
                    args.local_results_path, "models", "IC-MARL_" + args.env + f"_mode{args.train_mode}", str(
                        runner.t_env)
                )
            else:
                save_path = os.path.join(
