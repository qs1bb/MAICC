...
class EpisodeRunner:
    ...
    def run(self, test_mode=False):
        ...
        while not terminated:
            ...
            if self.args.name == "IC-MARL":
                actions = self.mac.select_actions(
                    self.batch, t_ep=self.t, t_env=len(self.test_returns) / self.args.test_nepisode, test_mode=test_mode
                )
            else:
                actions = self.mac.select_actions(
            ...
            if self.args.name == "IC-MARL":
                self.mac.update_context(self.batch, self.t)
        ...
        if self.args.name == "IC-MARL" and self.args.evaluate:
            print(np.sum(episode_return))

        if test_mode and (len(self.test_returns) == self.args.test_nepisode):
            if self.args.name == "IC-MARL" and self.args.evaluate:
                self._log_evaluate_icmarl(cur_returns, cur_stats, log_prefix)
            else:
    ...
    def _log_evaluate_icmarl(self, returns, stats, prefix):
        for i in range(self.args.test_nepisode):
            self.logger.log_stat(prefix + "return_mean", returns[i], i)
        returns.clear()
        stats.clear()
