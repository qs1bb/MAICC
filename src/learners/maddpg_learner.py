...
class MADDPGLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.critic_optimiser = Adam(
            params=self.critic_params, lr=self.args.lr)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        target_vals = self.target_critic(
            inputs[:, 1:], target_actions.detach())
        ...
        for t in range(batch.max_seq_length - 1):
            pi = self.mac.forward(batch, t=t).view(
                batch_size, 1, self.n_agents, -1)
