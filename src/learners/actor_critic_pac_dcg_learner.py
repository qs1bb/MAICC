...
class PACDCGLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.state_value_optimiser = Adam(
            params=self.state_value_params, lr=args.lr)
    ...
    def train_critic_sequential(self, batch, rewards, mask, terminated):
                ...
                for t in range(batch.max_seq_length - 1):
                    target_q_values.append(
                        self.target_critic.forward(
                            batch, t=t, actions=current_actions[:,
                                                                t], policy_mode=False
                        )
                    )
        ...
        if self.args.standardise_rewards:
            ...
            target_v = target_v * \
                th.sqrt(self.ret_ms_v.var) + self.ret_ms_v.mean
        ...
        for t in range(batch.max_seq_length - 1):
            # In double Q-learning, the actions are selected greedy w.r.t. mac
            greedy_actions.append(self.critic.forward(
                batch, t=t, policy_mode=False))
        greedy_actions = th.stack(greedy_actions, dim=1)
        max_q_a = []
        for i in range(self.n_agents):
            current_actions = copy.deepcopy(greedy_actions)
        ...
        running_log["q_taken_mean"].append(
            (q_curr * mask).sum().item() / mask_elems)
