...
class COMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        q_taken = th.gather(
            q_vals, dim=1, index=actions.reshape(-1, 1)).squeeze(1)
        pi_taken = th.gather(
            pi, dim=1, index=actions.reshape(-1, 1)).squeeze(1)
        pi_taken[mask == 0] = 1.0
    ...
    def _train_critic(
        ...
        targets_taken = th.gather(
            target_q_vals, dim=3, index=actions).squeeze(3)

        if self.args.standardise_returns:
            targets_taken = targets_taken * \
                th.sqrt(self.ret_ms.var) + self.ret_ms.mean

        targets = self.nstep_returns(
            rewards, mask, targets_taken, self.args.q_nstep)
        ...
        running_log["target_mean"].append(
            (targets * mask).sum().item() / mask_elems)
        ...
        for t_start in range(rewards.size(1)):
            ...
            for step in range(nsteps + 1):
                ...
                if t >= rewards.size(1):
                    ...
                    nstep_return_t += self.args.gamma**step * \
                        values[:, t] * mask[:, t]
