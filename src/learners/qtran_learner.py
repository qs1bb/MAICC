...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        target_mac_out = th.stack(
            target_mac_out[:], dim=1)  # Concat across time
        ...
        if self.args.mixer == "qtran_base":
            ...
            nopt_values = (
                chosen_action_qvals.sum(
                    dim=2).reshape(-1, 1) - joint_qs.detach() + vs
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.mixer == "qtran_base":
                ...
                self.logger.log_stat(
                    "td_targets", ((masked_td_error).sum(
                    ).item() / mask_elems), t_env
                )
                self.logger.log_stat(
                    "td_chosen_qs", (joint_qs.sum().item() / mask_elems), t_env
                )
                self.logger.log_stat(
                    "v_mean", (vs.sum().item() / mask_elems), t_env)
                self.logger.log_stat(
                    "agent_indiv_qs",
                    (
