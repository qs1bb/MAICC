...
SMACv2_CONFIG_DIR = Path(__file__).parent.parent / \
    "config" / "envs" / "smacv2_configs"
...
def load_scenario(map_name, **kwargs):
    ...
    if map_name == "smac_test":
        race = random.randint(0, 2)
        num_allies = random.randint(3, 12)
        num_enemies = num_allies
        if race == 0:
            scenario_args["env_args"].update({"map_name": "10gen_protoss"})
            scenario_args["env_args"]["capability_config"]["team_gen"].update(
                {"unit_types": ["stalker", "zealot", "colossus"]})
        elif race == 1:
            scenario_args["env_args"].update({"map_name": "10gen_terran"})
            scenario_args["env_args"]["capability_config"]["team_gen"].update(
                {"unit_types": ["marine", "marauder", "medivac"]})
        elif race == 2:
            scenario_args["env_args"].update({"map_name": "10gen_zerg"})
            scenario_args["env_args"]["capability_config"]["team_gen"].update(
                {"unit_types": ["zergling", "hydralisk", "baneling"]})
        scenario_args["env_args"]["capability_config"].update(
            {"n_units": num_allies})
        scenario_args["env_args"]["capability_config"].update(
