...
class WeightedTeamsDistribution(Distribution):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        if self.ally_team and self.enemy_team:
            team = self.ally_team
...
class ReflectPositionDistribution(Distribution):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self.n_enemies >= self.n_units:
            enemy_positions[: self.n_units, 0] = self.map_x - \
                ally_positions[:, 0]
            ...
            enemy_positions[:, 0] = self.map_x - \
                ally_positions[: self.n_enemies, 0]
...
class SurroundedAndReflectPositionDistribution(Distribution):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        if p > self.p_threshold:
            self.position = self.surrounded_distribution.generate()
        else:
            self.position = self.reflect_distribution.generate()
...
class RangedRandomPositionDistribution(Distribution):
    def __init__(self, config):
        self.config = config
        self.n_units = config["n_units"]
        self.n_enemies = config["n_enemies"]
        ...
        self.map_y = config["map_y"]
        self.rng = default_rng()
        self.ally_positions = None
        self.enemy_positions = None

    def generate(self) -> Dict[str, Dict[str, Any]]:
        if self.ally_positions is None or self.enemy_positions is None:
            ally_position = np.zeros((self.n_units, 2))
            enemy_position = np.zeros((self.n_enemies, 2))

            for i in range(self.n_units):
                x = self.rng.uniform(self.map_x * 1 / 8, self.map_x * 3 / 8)
                y = self.rng.uniform(self.map_y * 3 / 8, self.map_y * 5 / 8)
                ally_position[i][0] = x
                ally_position[i][1] = y

            for i in range(self.n_enemies):
                x = self.rng.uniform(self.map_x * 5 / 8, self.map_x * 7 / 8)
                y = self.rng.uniform(self.map_y * 3 / 8, self.map_y * 5 / 8)
                enemy_position[i][0] = x
                enemy_position[i][1] = y

            self.ally_positions = ally_position
            self.enemy_positions = enemy_position
            ...
            return {
                "ally_start_positions": {"item": self.ally_positions, "id": 0},
                "enemy_start_positions": {"item": self.enemy_positions, "id": 0},
            }
...
register_distribution(
    "random", RangedRandomPositionDistribution
)
